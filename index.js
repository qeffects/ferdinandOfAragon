...
const loadGuild = (gld) => {
  jsonfile
    .readFile(path.resolve(storagePath, `${gld.id}.json`))
    .then((obj) => {
      guilds[gld.id] = obj;
    ...
    .catch((e) => {
      if (e.code === 'ENOENT') {
        const initialGldData = {
          status: 'setup',
          messages: [],
        };

        jsonfile.writeFile(
          path.resolve(storagePath, `${gld.id}.json`),
          initialGldData
        );

        guilds[gld.id] = initialGldData;
        guilds[gld.id].guild = gld;
      }
    });
};
...
const saveGuild = (gld) => {
  jsonfile
    .writeFile(
      path.resolve(storagePath, `${gld.guild.id}.json`),
      {
        status: gld.status,
...
const onMessageHandler = (message) => {
  ...
  if (gld.status === 'setup' && message.member.hasPermission('ADMINISTRATOR')) {
    if (message.content.startsWith('%help')) {
      ...
      message.channel.send(
        `Inkvizīcijas kanāls uzstādīts ${message.mentions.channels.first(1)[0]}`
      );
      if (gld.inquisition_channel && gld.inquisition_role) {
        ...
        message.channel.send(
          'Inkvizīcija gatava, sāc rakstīt jautājumus ar %ask <jautājums>, nosauc nākamo inkvizējamo ar %target @<user>, un sāc inkvizīciju ar %start (apskati pilnās insturkcijas ar %help)'
        );
      }
      ...
      message.channel.send(
        `Inkvizīcijas role uzstādīta ${message.mentions.roles.first(1)[0]}`
      );
      if (gld.inquisition_channel && gld.inquisition_role) {
        console.log('ready');
        gld.status = 'ready';
        ...
        message.channel.send(
          'Inkvizīcija gatava, sāc rakstīt jautājumus ar %ask <jautājums>, nosauc nākamo inkvizējamo ar %target @<user>, un sāc inkvizīciju ar %start (apskati pilnās instrukcijas ar %help)'
        );
      }
    }
  }
  ...
  if (
    (gld.status === 'ready' || gld.status === 'in-progress') &&
    ...
    if (message.content.startsWith('%help')) {
      ...
      if (gld.status === 'in-progress') {
        if (
          findChannel(gld, gld.inquisition_channel)?.lastMessage?.author ==
            client.user &&
          findChannel(
            gld,
            gld.inquisition_channel
          )?.lastMessage?.content.startsWith('**Pagaidām jautājumu nav') &&
      ...
      message.channel.send(
        `Jauns inkvizīcijas mērķis ${message.mentions.members.first(1)[0]}`
      );
      ...
      let finStr = `Jautājumu rinda (${Math.min(
        numToShow,
        gld.messages.length
      )}/${gld.messages.length}):
`;
...
`;
      ...
      message.channel.send(
        `Inkvizīcija sākta! Jautājumu skaits: ${gld.messages.length}`
      );
  ...
  if (
    gld.status === 'in-progress' &&
    message.channel.id === gld.inquisition_channel
  ) {
